...
const UPLOAD_DIR = path.join(__dirname, '..', '..', 'uploads', 'product-categories');
...
async function compressAndSave(req, next) {
  try {
    ...
    req.body.image = `/uploads/product-categories/${filename}`;
    next();
  } catch (err) {
    next(err);
  }
...
function uploadProductCategoryImage(req, res, next) {
  const isMultipart = (req.headers['content-type'] || '').includes('multipart/form-data');
  if (!isMultipart) return next();
  runMulter(req, res, next);
}

module.exports = uploadProductCategoryImage;
