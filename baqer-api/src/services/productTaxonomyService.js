...
const { Category, ProductSubcategory, Brand, Product } = require('../models');
...
async function listAll() {
  const [categories, subcategories, brands] = await Promise.all([
    ...
    Brand.find().sort({ order: 1, nameAr: 1 }).lean(),
  ]);
...
async function createBrand(body) {
  ...
  if (!category) throw notFound('التصنيف الرئيسي غير موجود');
  const nameAr = String(body.nameAr || '').trim();
  const exists = await Brand.findOne({ categoryId: body.categoryId, nameAr }).lean();
