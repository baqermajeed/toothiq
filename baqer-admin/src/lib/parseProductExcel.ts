...
export function parseIraqiPrice(raw: unknown): number | null {
  ...
  if (thousandsDot) return Number(thousandsDot[1].replace(/\./g, ''))

  const thousandsComma = s.match(/^(\d{1,3}(?:,\d{3})+)\.*$/)
  if (thousandsComma) return Number(thousandsComma[1].replace(/,/g, ''))
